...
var OData = (function () {
    ...
    OData.prototype._handleGet = function (req, res) {
        var _this = this;
        this.checkAccess(req, res, odata_enums_1.HttpMethod.GET).then(function () {
            _this.oDataGet.handleGet(req, res);
        }).catch(function (err) {
    ...
    OData.prototype._handlePost = function (req, res) {
        var _this = this;
        this.checkAccess(req, res, odata_enums_1.HttpMethod.POST).then(function () {
            _this.oDataPost.handlePost(req, res);
        }).catch(function (err) {
            var statusCode = (err && err.statusCode) || 500;
            res.status(statusCode).send(err);
        });
    ...
    OData.prototype.checkAccess = function (req, res, method) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            var remotes = _this.oLoopbackApp.remotes();
            common.getRequestModelClass(_this.oLoopbackApp.models, req.params[0]).then(function (modelClassResult) {
                var ctx;
                for (var _i = 0, _a = remotes.classes(); _i < _a.length; _i++) {
                    var lbClass = _a[_i];
                    if (lbClass.name === modelClassResult.modelClass.modelName) {
                        if (method === odata_enums_1.HttpMethod.GET) {
                            ctx = _this.getGETCheckAccessContext(req, lbClass);
                            break;
                        }
                        else if (method === odata_enums_1.HttpMethod.POST) {
