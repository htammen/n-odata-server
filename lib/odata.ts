...
export class OData {
    ...
    private getGETCheckAccessContext(req:LoopbackRequest, lbClass:any) {
        ...
        let re = /^\w*[(]([a-zA-Z0-9']*)[)]/g,
            ...
            lbMethod;
        if (match) {
            lbMethod = lbClass.findMethodByName("findById");
            _id = match[1];
        } else {
            lbMethod = lbClass.findMethodByName("find");
        }
    ...
    private getPOSTCheckAccessContext(req:LoopbackRequest, lbClass:any) {
        ...
        let lbMethod;
        lbMethod = lbClass.findMethodByName("create");
    ...
    private getPUTCheckAccessContext(req:LoopbackRequest, lbClass:any) {
        ...
        if (match) {
            lbMethod = lbClass.findMethodByName("updateAttributes");
            _id = match[1];
        }
    ...
    private getDELETECheckAccessContext(req:LoopbackRequest, lbClass:any) {
        ...
        if (match) {
            lbMethod = lbClass.findMethodByName("destroyById");
            _id = match[1];
        }
    ...
    private getAccessTokenInstance(req:LoopbackRequest, res:express.Response) {
        return new Promise<any>((resolve, reject) => {
            let sAccessToken:String;

            //access token can come in header or query parameter
            if (typeof req.headers["authorization"] !== "undefined") {
                sAccessToken = req.headers["authorization"];
            } else if (typeof req.query.accessToken !== "undefined") {
                sAccessToken = req.query.accessToken;
            }
            else {
                let err = new Error(`The request not have an accessToken - model: ${req.params[0]}.`) as any;
                err.statusCode = 401;
                reject(err);
            }

            this.oLoopbackApp.models.AccessToken.findById(sAccessToken, {}, function (err, instance) {
                if (!err) {
                    req.accessToken = instance;
                    resolve();
                } else {
                    reject(err);
                }
